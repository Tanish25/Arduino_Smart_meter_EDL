...
Tanish H Talapaneni
    Attachments 10 : 32 AM(0 minutes ago)
        to me
...
volatile uint16_t Overall_Sum_Power = 0;
...
uint8_t max_val_voltage = 0;
uint8_t threshold_voltage = 0;
uint8_t threshold_current = 0;
uint8_t max_val_current = 0;
uint8_t max_val_power = 0;
...
void adc_read()
{
  ...
  if (f % 4 < 2)
  {
    ...
    if (f % 4 == 1)
    {
      ...
      uint8_t max_val_voltage = Voltage[0];
  ...
  else
  {
    ...
    if (f % 4 == 3)
    {
      ...
      uint8_t max_val_current = Current[0];
      ...
      Overall_Sum_Power += poww[pow_count] / 100; // change here

      uint8_t max_val_power = poww[0];
      if (poww[pow_count] > max_val_current)
      {
